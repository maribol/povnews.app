...
Hard rules for how news candidates are filtered and how digest items are framed. Phase 1 uses these to disqualify candidates. Phase 2 uses them when writing `whyItMatters` ("what this means for you") and `summary`.
...
- specific enough that `whyItMatters` could only apply to **this user** — not a generic segment or analyst take
...
## Rule 4 — `whyItMatters` = what this means for you

`whyItMatters` is the user-facing **"What this means for you"** line. It must answer: **what changes for this user's work because of this news?**

Write to the user when natural ("you", "your stack", "your tests"). Do **not** write analyst segmentation ("maps to performance teams…", "relevant for operators who…") without a direct implication.

| Bad `whyItMatters` | Better `whyItMatters` |
|--------------------|----------------------|
| "Controlled automation granularity maps to teams juggling SKU catalogs and creative throughput." | "If catalog scale is your bottleneck, finer automation beats another default-to-platform workflow." |
...
- [ ] `whyItMatters` reads as **what this means for you** — direct, user-specific, not analyst segmentation
